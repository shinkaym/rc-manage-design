...
import type { ChartPoint } from '@/features/report/report-data';
...
export function LineChart({
  ...
  const availablePlotWidth =
    containerWidth > 0
      ...
      : fallbackPlotWidth;
  const minimumPlotWidth = points.length <= visiblePointCount
    ? availablePlotWidth
    : Math.max(availablePlotWidth, Math.max(points.length - 1, 1) * 52);
  const svgWidth =
    chartLayout.axisLabelWidth + minimumPlotWidth + chartLayout.canvasRightPadding;
  ...
  const coordinates = useMemo(() => {
      ...
      return [
        {
          ...
          y:
            chartLayout.plotTopPadding +
            (1 - points[0].value / resolvedMaxY) * plotHeight,
        },
      ];
    }
    ...
    return points.map((point, index) => ({
      ...
      y:
        chartLayout.plotTopPadding +
        (1 - point.value / resolvedMaxY) * plotHeight,
    }));
